/*-----------------------------------------------------------------------------------
 ...
 Theme Name: The Goodini eCommerce HTML Template
 Author: BigSteps
		...
		init: function () {
			...
			if ($('.homepage-select').length) {
				var $homepageSelect = $('.homepage-select'),
					$slide = $('#homepageSlide'),
					scrollenable = true,
					blank = '/img/onlydemo/layout-screen-000.png',
				...
				function _elevateZoom(el) {
					...
					if (!$('body').hasClass('touch')) {
						...
						$zoomImg.ezPlus({
							...
							imageCrossfade: true,
							responsive: true,
							loadingIcon: '/img/ajax_loader.gif',
							cursor: 'crosshair'
						});
		...
		instaFeed: function (el) {
			$(el).each(function () {
				...
				var userFeed = new Instafeed({
					...
					template: '<a href="{{link}}" target="_blank"><span><img src="/img/placeholder.png" data-src="{{image}}" class="lazyload" /></span></a>'
				});
				userFeed.run();
